...
const Dashboard = () => {
  ...
  const { user, credits, signOut, refreshCredits, daysUntilExpiry, foundingUser, hasPurchased, isPremiumOverride } = useAuth();
  const isPremium = hasPurchased || foundingUser || isPremiumOverride;
  ...
  if (selectedSession) {
    ...
    return (
      <ResponsiveShell>
        <div className="flex min-h-[100dvh] md:min-h-0 flex-col px-6 pt-8 pb-8 overflow-y-auto">
          <div className="flex items-center gap-3">
            <button onClick={() => setSelectedSession(null)} className="flex h-10 w-10 items-center justify-center rounded-2xl bg-surface ease-presence transition-transform active:scale-95 hover:bg-accent">
              <ChevronLeft className="h-5 w-5 text-foreground" />
            </button>
            <h2 className="text-lg font-semibold text-foreground md:text-xl">Session Details</h2>
          </div>
          ...
          <div className="mt-6 flex justify-center">
            <div className="relative flex h-28 w-28 items-center justify-center md:h-36 md:w-36">
              ...
              <div className="text-center">
                <span className="tabular-nums text-3xl font-semibold text-foreground md:text-4xl">{selectedSession.overall_score || 0}</span>
            ...
            <div className="mt-6 flex flex-col gap-3">
              {SKILL_KEYS.map((key) => (
                <div key={key}>
                  ...
                  {feedback[key] && <p className="ml-0 mt-1 text-[11px] leading-relaxed text-muted-foreground md:text-xs">{feedback[key]}</p>}
                </div>
              ))}
            </div>
          )}
          ...
          {tips.length > 0 && (
            <div className="mt-6 rounded-3xl bg-surface p-5 card-depth">
              ...
              <ul className="flex flex-col gap-2">
                {tips.map((tip, i) => (
                  <li key={i} className="text-xs leading-relaxed text-muted-foreground md:text-sm">{tip}</li>
                ))}
              </ul>
            </div>
          )}
  ...
  return (
    <ResponsiveShell>
      <div className="flex min-h-[100dvh] md:min-h-0 flex-col pt-8 pb-0">
        {/* Header */}
        <div className="flex items-center gap-3 px-6">
          <button onClick={() => navigate("/")} className="flex h-10 w-10 items-center justify-center rounded-2xl bg-surface ease-presence transition-transform active:scale-95 hover:bg-accent">
            <ChevronLeft className="h-5 w-5 text-foreground" />
          </button>
          <h2 className="text-lg font-semibold text-foreground md:text-xl">Dashboard</h2>
        </div>

        {/* Tabs */}
        <div className="mt-4 flex gap-1 px-6 overflow-x-auto">
          {tabs.map((tab) => (
            <button
              key={tab.id}
              onClick={() => setActiveTab(tab.id)}
              className={`flex-1 flex items-center justify-center gap-1 rounded-xl py-2 text-[11px] font-medium transition-colors hover:opacity-80 min-w-0 ${
          ...
          {activeTab === "progress" && (
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="mt-6 flex flex-col gap-5">
              ...
              <div className="grid grid-cols-3 gap-3">
                <div className="rounded-2xl bg-surface p-3 text-center card-depth md:p-4">
                  <p className="tabular-nums text-2xl font-semibold text-foreground md:text-3xl">{totalSessions}</p>
                  <p className="text-[10px] text-muted-foreground md:text-xs">Sessions</p>
                </div>
                <div className="rounded-2xl bg-surface p-3 text-center card-depth md:p-4">
                  <p className="tabular-nums text-2xl font-semibold text-foreground md:text-3xl">{avgScore}</p>
                  <p className="text-[10px] text-muted-foreground md:text-xs">Avg Score</p>
                </div>
                <div className="rounded-2xl bg-surface p-3 text-center card-depth md:p-4">
                  <p className="tabular-nums text-2xl font-semibold text-foreground md:text-3xl">{latestScore}</p>
                  <p className="text-[10px] text-muted-foreground md:text-xs">Latest</p>
                </div>
              </div>

              {/* Skill Breakdown */}
              <div className="rounded-3xl bg-surface p-5 card-depth">
                <h3 className="text-sm font-semibold text-foreground mb-4 md:text-base">Skill Breakdown</h3>
                <div className="flex flex-col gap-3">
                  {skillAverages.map((s) => (
                    <div key={s.key}>
                      <div className="flex items-center gap-2">
                        <span className="w-20 text-xs text-foreground md:w-24 md:text-sm">{s.label}</span>
                      ...
                      {isPremium ? (
                        <p className="ml-0 mt-1 text-[10px] leading-relaxed text-muted-foreground/70 md:text-xs">
              ...
              {sessions.length === 0 ? (
                ...
                sessions.map((s) => (
                  <button
                    ...
                    className="flex items-center justify-between rounded-2xl bg-surface p-4 card-depth text-left ease-presence transition-transform active:scale-[0.98] hover:bg-accent"
                  >
                    <div>
          ...
          {activeTab === "account" && (
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="mt-6 flex flex-col gap-4">
              ...
              <button
                ...
                className="flex h-12 w-full items-center justify-center gap-2 rounded-2xl bg-surface text-foreground font-medium ease-presence transition-transform active:scale-95 hover:bg-accent ring-1 ring-border"
              >
                Change Password
              </button>
              ...
              <button
                ...
                className="flex h-12 w-full items-center justify-center gap-2 rounded-2xl bg-destructive text-destructive-foreground font-medium ease-presence transition-transform active:scale-95 hover:opacity-90"
              >
